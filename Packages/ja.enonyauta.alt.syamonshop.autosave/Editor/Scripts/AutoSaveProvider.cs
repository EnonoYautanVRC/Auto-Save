		...
		public static SettingsProvider CreateCustomProvider()
		{
			SettingsProvider tmpProvider = new SettingsProvider(c_SETTING_PATH, SettingsScope.User)
			{
				...
				guiHandler = searchContext =>
				{
					...
					/** Auto Save */
					m_IsAutoSave = EditorGUILayout.BeginToggleGroup("自動保存 & バックアップ / Auto Save & Backup", m_IsAutoSave);   // Group-AutoSave

					EditorGUILayout.Space();    // Space

					/** Timer */
					EditorGUILayout.LabelField("保存間隔(分) / Save Scene Interval (min) [1～1,440]");
					m_Interval = EditorGUILayout.IntSlider("", m_Interval, 1, 1440);

					EditorGUILayout.Space();    // Space

					/** History Quantity */
					EditorGUILayout.LabelField("バックアップ数上限 / History Quantity [1～500]");
					m_Quantity = EditorGUILayout.IntSlider("", m_Quantity, 1, 500);
